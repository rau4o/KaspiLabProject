...
class CustomTabbarItem: UIView {
    ...
    private func drawConstraints() {
        ...
        NSLayoutConstraint.activate([
            ...
            tabImageView.heightAnchor.constraint(equalToConstant: CustomTabbarSetting.tabBarSizeImage),
            tabImageView.widthAnchor.constraint(equalToConstant: CustomTabbarSetting.tabBarSizeImage),
            titleLabel.bottomAnchor.constraint(equalTo: self.bottomAnchor, constant: CustomTabbarSetting.tabBarHeight),
   ...
   internal func animateTabSelected() {
        ...
        UIView.animate(withDuration: CustomTabbarSetting.tabBarAnimationDurationTime) { [weak self] in
            ...
            self?.titleLabel.frame.origin.y = CustomTabbarSetting.tabBarHeight / 1.8
    ...
    internal func animateTabDeSelect() {
        tabImageView.alpha = 1
        UIView.animate(withDuration: CustomTabbarSetting.tabBarAnimationDurationTime) { [weak self] in
            self?.titleLabel.frame.origin.y = CustomTabbarSetting.tabBarHeight
            self?.tabImageView.frame.origin.y = (CustomTabbarSetting.tabBarHeight / 2) - CGFloat(CustomTabbarSetting.tabBarSizeImage / 2)
